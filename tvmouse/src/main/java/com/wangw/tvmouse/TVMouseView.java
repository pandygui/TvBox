...
public class TVMouseView extends FrameLayout {
    ...
    // 鼠标的资源文件
    private int pointerResource = R.mipmap.cursor;
    // 鼠标的大小
    ...
    public TVMouseView(@NonNull Context context) {
        super(context);
    }

    // 设置用于传递点击事件的目标View（即传给鼠标View的Parent ViewGroup）
    TVMouseView(Context context, View targetView) {
        super(context);
    ...
    private Bitmap drawableToBitmap() {
        Resources resources;
        if (pointerResource == R.mipmap.cursor) {
